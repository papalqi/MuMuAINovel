...
**9. 改进建议（必须“可直接用来重写”）**
建议数量必须与整体质量分数关联（但始终要给足可执行建议）：
- overall < 4.0：提供6-8条建议（至少3条为P0，指出严重问题）
- overall 4.0-5.9：提供5-7条建议（至少2条为P0，指出主要问题）
- overall 6.0-7.9：提供4-6条建议（至少1条为P0，指出关键短板）
- overall ≥ 8.0：提供3-5条建议（以P1/P2为主，允许最多1条P0）

每条建议必须是一条“可执行指令”，并严格使用以下格式（建议输出为字符串）：
【P0|维度】位置:开头/中段/结尾/某场景｜证据:「逐字引用8-25字」｜问题:...｜影响:...｜动作:...｜示例改写:...(1-3句)

补充要求：
- **证据**必须来自本章原文，逐字复制，长度8-25字（用于精确定位）
- **动作**必须可量化/可操作（如“删减到X字”“补一段冲突对话”“在结尾追加一个未解问题/反转信息”）
- **示例改写**必须贴合本章语气与已知设定，不要引入新角色/新设定/跳跃时间线
- 建议应尽量覆盖：节奏(pacing)、吸引力/钩子(engagement)、连贯性/动机(coherence) 三个方向；若无明显问题，则给“锦上添花”的具体增强手段
</analysis_framework>
...
{{
  ...
  "plot_stage": "发展",
  "suggestions": [
    "【P0|节奏】位置:中段｜证据:「从原文逐字复制的8-25字文本」｜问题:心理描写过长导致推进停滞｜影响:紧张感下降、阅读动力变弱｜动作:将该段压缩到约120-180字，并用一句动作或对话承接到下一情节点｜示例改写:「他强迫自己冷静下来，抬眼盯住对方：‘你说的证据，到底在哪？’」",
    "【P1|钩子】位置:结尾｜证据:「从原文逐字复制的8-25字文本」｜问题:结尾缺少明确悬念/反转收束｜影响:读者缺少“必须点下一章”的理由｜动作:在收尾追加一个未解问题或信息差（与本章冲突/伏笔直接相关）｜示例改写:「门缝里那张纸条只写了四个字——‘别相信他’。」",
    "【P1|连贯性】位置:开头｜证据:「从原文逐字复制的8-25字文本」｜问题:角色动机转变缺少铺垫，行为显得突兀｜影响:人物可信度下降、前后不一致｜动作:在转折前补1-2句“理由/代价/触发点”，让决定看起来是被逼出来的｜示例改写:「他本想退一步，可一想到昨夜的警告，指尖还是不由自主攥紧了。」"
  ]
}}
</output>
...
✅ 建议数量必须与overall分数关联（同时保证建议“可执行”）：
   - overall < 4.0 → 6-8条建议（至少3条P0）
   - overall 4.0-5.9 → 5-7条建议（至少2条P0）
   - overall 6.0-7.9 → 4-6条建议（至少1条P0）
   - overall ≥ 8.0 → 3-5条建议（最多1条P0）
✅ 每条建议必须以【P0|维度】/【P1|维度】/【P2|维度】开头
✅ 每条建议必须包含：位置、证据（8-25字逐字引用）、问题、影响、动作、示例改写（1-3句）
